...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="animate-pulse-glow pointer-events-none absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] rounded-full"
        style={{
          background:
            "radial-gradient(ellipse at center, rgba(124,58,237,0.22) 0%, rgba(6,182,212,0.10) 45%, transparent 70%)",
          filter: "blur(60px)",
        }}
      />
      <div
      ...
      <motion.h1
        {...fadeUp(0.2)}
        className="text-center text-5xl sm:text-6xl lg:text-7xl font-extrabold leading-tight tracking-tight max-w-4xl"
      >
        Build AI Apps{" "}
      ...
      <motion.p
        {...fadeUp(0.35)}
        className="mt-6 text-center text-lg sm:text-xl text-slate-400 max-w-2xl leading-relaxed"
      >
        NeuralAI gives your team a production-ready AI platform — from natural
        language APIs to auto-scaling inference — so you ship in hours, not
        months.
      </motion.p>
      ...
      <motion.ul
        {...fadeUp(0.45)}
        className="mt-5 flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm text-slate-400"
      >
      ...
      >
        <div className="relative rounded-2xl border border-slate-700/60 bg-slate-900/80 backdrop-blur-sm shadow-2xl shadow-violet-900/20 overflow-hidden">
          ...
          <div className="flex items-center gap-2 px-5 py-3.5 border-b border-slate-800 bg-slate-900/90">
            ...
            <span className="text-slate-600 text-xs font-mono">neuralai.dev/playground</span>
          </div>

          {/* Code editor area */}
          <div className="p-5 sm:p-8 font-mono text-xs sm:text-sm leading-relaxed relative min-h-[280px]">
            ...
            {!playing && (
              <motion.button
                ...
                className="absolute inset-0 flex items-center justify-center bg-slate-950/60 backdrop-blur-[2px] group"
                aria-label="Run demo"
              >
                <span className="flex flex-col items-center gap-3">
                  <span className="w-16 h-16 rounded-full bg-gradient-to-br from-violet-600 to-cyan-500 flex items-center justify-center shadow-xl shadow-violet-700/50 group-hover:shadow-violet-500/70 group-hover:scale-105 transition-all duration-300">
